...
class LoginScreen extends React.Component {
  static navigationOptions = {
    header: ({ state, setParams }) => ({
      title: state.params ? state.params.title : 'Login Screen' ,
      visible: state.params ? state.params.visible : true ,
    })
  }
  ...
  render() {
    ...
    const visible = params && params.visible;
    return (
      <View style={styles.container}>
        <Text style={styles.welcome}>
          Screen A
        ...
        <Button
          onPress={() => this.props.navigation.setParams({ title: 'New header', visible: !visible })}
          title="set header"
        />
      </View>
    )
...
const LoginStatusMessage = connect(state => ({
  ...
  return (
    <View>
      ...
      <Button
        onPress={() => navigation.setParams({ title: 'paco' })}
        title="Set header"
      />
    </View>
  );
...
const AuthButton = connect(state => ({
  ...
  login: () => dispatch({ type: 'Navigate', routeName: 'Login', params: {visible: false} }),
...
AppRegistry.registerComponent('ForestWatcher', () => ReduxExampleApp);

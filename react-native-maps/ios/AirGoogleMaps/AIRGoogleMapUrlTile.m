...
- (void)_requestTileForX:(NSUInteger)x y:(NSUInteger)y zoom:(NSUInteger)zoom receiver:(id<GMSTileReceiver>)receiver {
  NSLog(@"Cord 222");
  UIGraphicsBeginImageContextWithOptions(CGSizeMake(36, 36), NO, 0.0);
  UIImage *blank = UIGraphicsGetImageFromCurrentImageContext();
  
  UIGraphicsEndImageContext();
  [receiver receiveTileWithX:x y:y zoom:zoom image:blank]; // pass it to the SDK API
}
@end

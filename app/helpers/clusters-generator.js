...
class ClustersGenerator {
  ...
  mapAreaToClusters = memoize(
    (areaId: string, slug: string, startDate: number) => {
      const realm = initDb();
      // TODO: use days in both systems
      const timeFrame = slug === DATASETS.VIIRS ? 'day' : 'month';
      const limitRange = moment()
        .subtract(startDate, timeFrame)
        .valueOf();
      const alerts = read(realm, 'Alert').filtered(
        `areaId = '${areaId}' AND slug = '${slug}' AND date > '${limitRange}'`
      );
